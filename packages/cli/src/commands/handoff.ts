...
import {
  codexHome,
  performClaudeHandoff,
  performHandoff,
  resumeCommandFor,
  type HandoffTarget,
} from '../handoff/perform.js';
...
export function registerHandoff(program: Command): void {
  program
    ...
    .option(
      '--cwd <path>',
      "map the session onto your local clone (a teammate's project path differs from yours)",
    )
    ...
    .action(
      async (
        ...
        if (opts.toMember) {
          ...
          const req = await client.createHandoffRequest({
            sessionId: resolved,
            cwd: opts.cwd,
            assignee: opts.toMember,
            target,
          });
        ...
        if (opts.dryRun && target === 'claude-code') {
          const preview = performClaudeHandoff(session, {
              ...
              JSON.stringify(
                {
                  ...
                  droppedReasoning: preview.droppedReasoning,
                },
                null,
                2,
              ),
        ...
        if (opts.open) {
          ...
          if (target === 'claude-code') {
            ...
            if (direct.error && (direct.error as NodeJS.ErrnoException).code === 'ENOENT') {
              spawnSync('npx', ['-y', '@openai/codex', 'resume', result.threadId], {
                cwd,
                stdio: 'inherit',
                shell,
              });
